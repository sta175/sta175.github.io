...
**filter:** One method to create a subset of data is the `filter` function, which specifies the rows of data we want to keep. For example, if we only want to keep the Chinstrap penguins in the `penguins` data, we would use
...
**count:** The `count` function counts the number of rows for each level of one or more variables. For example, to calculate the number of penguins of each species in the `penguins` data:
...
**group_by** and **summarize**: Suppose we want to calculate the mean body mass for each species of penguins. This calculation involves two steps: first we *group* the penguins by their species, then we *summarize* each species with their mean body mass. The `group_by` function can be used to group rows of the data by one or more explanatory variables, and the `summarize` function can be used to calculate summary statistics:
...
**mutate**: One way to make new columns in the data is with the `mutate` function. Suppose we want to make a new column in the penguins data which contains the ratio of body mass to flipper length:
...
We can see that the penguins data now contains this new column!
